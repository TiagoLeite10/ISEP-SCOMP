...
int main() {
    ...
    // Pede ao utilizador para inserir o id do cliente que pretende consultar os dados
    char id_to_find[10];
    printf("Please, insert the client id that you want to find: ");
    fgets(id_to_find, 10, stdin);
    ...
    sleep(1); // Simula o tempo de procura dos dados de 1 segundo
    ...
    int count = 0;
    int found = 0;
    while (count < functional_shared_data->num_clients && !found) { // Enquanto não encontra o utilizador nos dados validos dos clientes
        if (temp_clients_shared_data->id_number == val) { // Se encontrar o cliente desejado
            found = 1;
    ...
    if (!found) {
        printf("The cliente with id %d doesn't exist!\n", val);
    }
