...
int main() {
    ...
    // Se o máximo de clientes for atingido, não avançar
    if (functional_shared_data->num_clients == NUM_MAX_CLIENTS) {
        printf("The number max of clients was reached!\n");
        return 0;
    }
    ...
    // Cria e abre a zona de memória partilhada
    fd_clients = shm_open_with_validations(CLIENTS_SHARED_MEMORY_NAME, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
    // Se não der erro no ftruncate define o tamanho da área da memória partilhada e inicializa-a a 0
    ftruncate_with_validations(fd_clients, clients_data_size);
    // Pointer para a memória partilhada
    clients_shared_data = (client_data *) mmap_with_validations(NULL, clients_data_size, PROT_WRITE | PROT_READ, MAP_SHARED, fd_clients, 0);
    ...
    // Pedir os dados do novo cliente a inserir
    client_data new_client = {0};

    printf("Insert id number: \n");
    char id_to_find[10];
    fgets(id_to_find, 10, stdin); // Ler o id
    int val = strtol(id_to_find, NULL, 10);
    if ((errno == ERANGE && (val == LONG_MAX || val == LONG_MIN)) || (errno != 0 && val == 0)) {
        perror("An error occurred while doing the strtol!\n");
        exit(EXIT_FAILURE);
    }
    new_client.id_number = val;

    printf("Insert cliente name: \n");
    fgets(new_client.name, 55, stdin); // Ler o nome
    strtok(new_client.name, "\n"); // Remover o newline

    printf("Insert client address: \n");
    fgets(new_client.address, 60, stdin); // Ler a morada
    strtok(new_client.address, "\n"); // Remover o newline

    sem_wait_with_validation(sem_torniquete); // Ganhar a prioridade de acesso à zona crítica
    sem_wait_with_validation(sem_no_use); // Ganhar o acesso absoluto e exclusivo à zona crítica

    // Colocar os dados do novo cliente na zona crítica
    client_data *temp_client;
    temp_client = clients_shared_data + functional_shared_data->num_clients;
    temp_client->id_number = new_client.id_number;
    strcpy(temp_client->name, new_client.name);
    strcpy(temp_client->address, new_client.address);
    functional_shared_data->num_clients++;

    sleep(5); // Simular que a operação de inserir demora 5 segundos

    sem_post_with_validation(sem_torniquete); // Libertar a prioridade de acesso à zona crítica
    sem_post_with_validation(sem_no_use); // Libertar o acesso absoluto e exclusivo à zona crítica
