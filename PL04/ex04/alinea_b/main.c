...
#define SEMAPHORE_NAME "/sem_ex04b_%d"
#define SEMAPHORE_INDEX_1 0
#define SEMAPHORE_INDEX_2 1
#define SEMAPHORE_INDEX_3 2
...
int main() {
    ...
    for (i = 0; i < NUMBER_OF_PROCESSES; i++) {
        ...
        if (p == 0) {
            for (j = 1; j <= NUMBER_OF_EXECUTIONS; j++) {                
                if (i == 0) {  
                    ...
                    /* Espera até que o semáforo tenha um valor maior que zero, para então decrementá-lo */
                    sem_wait_with_validation(sem[SEMAPHORE_INDEX_3]);

                    printf("2nd child\n");

                    /* Incrementa e liberta o semáforo do 1º filho */
    ...
    for (j = 0; j < NUMBER_OF_EXECUTIONS; j++) {
        ...
        /* Incrementa e liberta o semáforo do 2º filho */
        sem_post_with_validation(sem[SEMAPHORE_INDEX_3]);
    }
